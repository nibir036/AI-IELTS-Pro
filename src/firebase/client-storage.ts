    ...
    try {
        ...
        console.error("Client-side upload failed:", error);
         if (error.code === 'storage/unauthorized' || error.message?.includes('CORS')) {
            ...
            const detailedError = `Upload failed due to a CORS policy error. This is a server configuration issue. Please run 'gsutil cors set cors.json gs://${bucketName}' in your terminal to fix it.`;
